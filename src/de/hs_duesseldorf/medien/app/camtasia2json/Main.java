...
class Main {
	/**
	 * Aktiviert / Deaktiviert den Debug-Modus. Beim kompillieren einer Anwendung f�r den produktiven Einsatz immer auf
	 * false setzen!
	 * Im Debug-Modus wird das Ergebnis der Transformation direkt auf die Konsole ausgegeben. Es wird au�erdem nicht durch
	 * einen JSON Parser gepr�ft oder reformatiert.
	 */
	 ...
	 * optional den Pfad f�r die Ausgabedatei und optional den Pfad zum XSL Stylesheet.
	 * Die Eingabedateien werden eingelesen, gepr�ft und eine Transformation der Projektdatei in eine JSON Datei vorgenommen.
		...
		try {
			...
			if (bothFilesValid) {
				...
				try {
					...
					// Keine zus�tzliche Fehlermeldung ausgeben, wenn der Fehler von einer <xsl:message> ausgel�st wurde.
					if (!ex.getXMLError().getMessage(0).equals("TERMINATE PROCESSING"))
						throw new Camtasia2JsonException(String.format("Die XSL Transformation f�r die Datei \"%s\" ist fehlgeschlagen: %s\n", xslFileUrl, ex.getMessage()), ex);

					return;
				} catch (IOException ex) {
					throw new Camtasia2JsonException(String.format("I/O fehler bei der XSL Transformation f�r die Datei \"%s\": %s\n", xslFileUrl, ex.getMessage()), ex);
				}
				...
				try {
					...
					throw new Camtasia2JsonException(String.format("Bei der Pr�fung der JSON Daten: " + ex.getMessage()), ex);
				} catch (IOException ex) {
					if (outputToFile)
	...
	/**
	 * Pr�ft einen gegebenen Dateinamen auf G�ltigkeit und wenn dort eine Datei exestiert, ob in diese geschrieben werden kann.
	 * Gibt anschlie�end den gegebenen Dateinamen mit einem absoluten Pfad zur�ck.
	 * @param destFileName Der Dateiname der gepr�ft werden soll.
	 * @return Der Eingabedateiname mit absolutem Pfad. Null, falls der Pfad ung�ltig war.
	 */
	private static String properCheckedDestFileName(String destFileName) throws Camtasia2JsonException {
		try {
			File file = new File(destFileName);
			if (file.exists() && !file.canWrite())
				throw new Camtasia2JsonException(String.format("Die Zieltdatei \"%s\" ist schreibgesch�tzt.", destFileName));

			return file.getCanonicalPath();
		} catch (IOException ex) {
			throw new Camtasia2JsonException(String.format("\"%s\" ist kein g�ltiger Pfad.", destFileName), ex);
		}
	}
	...
	private static XMLDocument xmlDocumentFromFile(@NotNull URL fileUrl) throws Camtasia2JsonException {
		...
		try {
			...
			throw new Camtasia2JsonException(String.format("Die Datei \"%s\" enth�lt ung�ltige XML Daten: %s Zeile: %d, Spalte: %d.\n", fileUrl, ex.getMessage(), ex.getLineNumber(0), ex.getColumnNumber(0)), ex);
		} catch (SAXException ex) {
			throw new Camtasia2JsonException(String.format("Die Datei \"%s\" enth�lt ung�ltige XML Daten: %s\n", fileUrl, ex.getMessage()), ex);
		} catch (IOException ex) {
	...
	/**
	 * Gibt ein URL Objekt zur�ck. Wenn die angegebene URL Zeichenfolge keine g�ltige URL ist, wird sie in eine g�ltige
	...
	private static void printHelpText() {
		...
		System.out.println("                  Bei Nichtangabe erfolgt Ausgabe �ber stdout.");
